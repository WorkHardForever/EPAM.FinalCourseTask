...
namespace ProjectManagement.AspNetMvc.PL.HtmlHelpers
{
    public static class HtmlExtensions
    {
        ...
        //    {
        //        return new HelperResult ( writer => writer.Write(template));
        //    };
        ...
        public static IHtmlString RenderResources(this HtmlHelper htmlHelper, string type)
        {
            if (htmlHelper.ViewContext.HttpContext.Items[type] != null)
            {
                List<Func<object, HelperResult>> resources =
                    (List<Func<object, HelperResult>>) htmlHelper.ViewContext.HttpContext.Items[type];

                foreach (var resource in resources)
                {
                    if (resource != null)
                        htmlHelper.ViewContext.Writer.Write(resource(null) + Environment.NewLine);
                }
            }
