...
test('drag event gets emitted when clicking an item',
    function(t) {
        ...
        testCase('fails whenever invalid returns true',
            { which: 1 },
        ...
        function testCase(desc, eventOptions, options) {
            t.test(desc,
                function subtest(st) {
                    ...
                    st.plan(passes ? 4 : 1);
                    st.equal(drake.dragging,
                        passes,
                        desc + ': final state is drake is ' + (passes ? '' : 'not ') + 'dragging');
                    st.end();
...
test('when dragging, element gets a mirror image for show',
    function(t) {
        var div = document.createElement('div');
        var item = document.createElement('div');
        var drake = dragula([div]);
...
test('when dragging, mirror element gets appended to configured mirrorContainer',
    function(t) {
...
test('when dragging stops, element gets gu-transit class removed',
    function(t) {
        var div = document.createElement('div');
        var item = document.createElement('div');
        var drake = dragula([div]);
...
test('when dragging stops, body becomes selectable again',
    function(t) {
        var div = document.createElement('div');
        var item = document.createElement('div');
        var drake = dragula([div]);
...
test('when drag begins, check for copy option',
    function(t) {
        var div = document.createElement('div');
        var item = document.createElement('div');
